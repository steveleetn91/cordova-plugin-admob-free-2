...
- (void) __cycleRewardVideo {
    ...
    @synchronized(self.rewardedVideoLock) {
        if (!self.isRewardedVideoLoading) {
            ...
            if (!self.rewardVideoView){
                self.rewardVideoView = [GADFullScreenPresentingAd sharedInstance];
                self.rewardVideoView.delegate = self;

                [self.rewardVideoView loadRequest:[GADRequest request] loadWithAdUnitID:self.rewardVideoId];
            }
        }
    }
}
...
- (void) __cycleInterstitial {
    ...
    if (!self.interstitialView){
        //self.interstitialView = [[GADInterstitial alloc] loadWithAdUnitID:self.interstitialAdId];
...
- (void)rewardBasedVideoAd:(GADFullScreenPresentingAd *)rewardBasedVideoAd
   didRewardUserWithReward:(GADRewardedAd *)reward {
    NSString* obj = @"AdMob";
...
- (void)rewardBasedVideoAdDidReceiveAd:(GADFullScreenPresentingAd *)rewardBasedVideoAd {
    @synchronized(self.rewardedVideoLock) {
...
- (void)rewardBasedVideoAdDidOpen:(GADFullScreenPresentingAd *)rewardBasedVideoAd {
    [self fireEvent:@"" event:@"admob.rewardvideo.events.OPEN" withData:nil];
}

- (void)rewardBasedVideoAdDidStartPlaying:(GADFullScreenPresentingAd *)rewardBasedVideoAd {
    [self fireEvent:@"" event:@"admob.rewardvideo.events.START" withData:nil];
}

- (void)rewardBasedVideoAdDidClose:(GADFullScreenPresentingAd *)rewardBasedVideoAd {
...
- (void)rewardBasedVideoAdWillLeaveApplication:(GADFullScreenPresentingAd *)rewardBasedVideoAd {
